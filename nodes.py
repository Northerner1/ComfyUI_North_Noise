...
class Unsampler:
    ...
    def INPUT_TYPES(s):
        return {"required":
                    ...
                    "noise_steps": ("INT", {"default": 20, "min": 0, "max": 10000}),
    ...
    def unsampler(self, model, cfg, sampler_name, steps, noise_steps, scheduler, normalize, positive, negative, latent_image, strength):
        ...
        for step in range(noise_steps):
            sigma = sigmas[step]
            noise = torch.randn_like(current_latent) * sigma * strength
